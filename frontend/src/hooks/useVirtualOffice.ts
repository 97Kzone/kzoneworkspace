...
export const useVirtualOffice = () => {
  ...
  const [activeTab, setActiveTab] = useState<'LOGS' | 'REASONING' | 'STATS' | 'SCHEDULER' | 'KANBAN' | 'TECH_PULSE' | 'ANALYTICS' | 'MISSION' | 'CODE_REVIEW' | 'JANITOR' | 'MISSION_CONTROL' | 'KNOWLEDGE' | 'STRATEGIC_COUNCIL' | 'EFFICIENCY' | 'ALIGNMENT' | 'EVALUATION_LAB' | 'STANDUP'>('LOGS');
