...
import { Heart, ShieldAlert, Zap, TrendingUp, Loader2, Sparkles } from "lucide-react";
import { projectHealthService, ProjectHealth, ActionableStrategy } from "../app/apiService";
...
export const ProjectHealthDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.error("Failed to fetch project health report:", e);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleAdoptStrategy = (strategy: ActionableStrategy) => {
    console.log("Adopting strategy:", strategy);
    alert(`전략 '${strategy.title}'이(가) 선택되었습니다.`);
  };
    ...
    <div className="flex-1 flex flex-col gap-6 overflow-hidden">
      <div className="flex items-center justify-between shrink-0 px-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest">프로젝트 건강진단 대시보드</h3>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">AI 기반 프로젝트 전략 분석 및 실시간 진단</p>
          </div>
        </div>
                ...
                {report.recommendations.map((strat, i) => (
                  ...
                  >
                    ...
                    <div className="pt-6 border-t border-slate-50 flex items-center justify-between">
                      ...
                      <button 
                        onClick={() => handleAdoptStrategy(strat)}
